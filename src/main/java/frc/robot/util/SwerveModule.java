...
public class SwerveModule {
    ...
    SimpleMotorFeedforward feedforward = new SimpleMotorFeedforward(DriveMap.DRIVE_KS, DriveMap.DRIVE_KV,
            DriveMap.DRIVE_KA);
    ...
    private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
        if (isOpenLoop) {
            ...
            double velocity = Conversions.MPSToFalcon(desiredState.speedMetersPerSecond, DriveMap.WHEEL_CIRCUMFERENCE,
    ...
    private void setAngle(SwerveModuleState desiredState) {
        Rotation2d angle = (Math.abs(desiredState.speedMetersPerSecond) <= (DriveMap.MAX_VELOCITY * 0.01)) ? lastAngle
                : desiredState.angle; // Prevent rotating module if speed is less then 1%. Prevents Jittering.

        rotator.set(ControlMode.Position,
    ...
    private Rotation2d getAngle() {
        return Rotation2d.fromDegrees(
                Conversions.falconToDegrees(rotator.getSelectedSensorPosition(), DriveMap.ANGLE_GEAR_RATIO));
    }
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(
                Conversions.falconToMPS(drive.getSelectedSensorVelocity(), DriveMap.WHEEL_CIRCUMFERENCE,
                        DriveMap.DRIVE_GEAR_RATIO),
                getAngle());
    }
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                Conversions.falconToMeters(drive.getSelectedSensorPosition(), DriveMap.WHEEL_CIRCUMFERENCE,
                        DriveMap.DRIVE_GEAR_RATIO),
                getAngle());
    }
}
